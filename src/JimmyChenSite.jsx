...
const THOUGHTS = [
  {
    ...
    category: "AI 推理芯片",
    readTime: "13 min",
    summary: "从海外巨头的训推分离与 PD 分离布局出发，梳理 Dojo、Groq、Cerebras、SoW 晶圆级互联、KV Cache 与 Decode 瓶颈，并进一步拆解昉擎科技以 Attention / FFN 异构部署重构推理基础设施的技术路径。",
    highlight: true,
    content: [
      { heading: "一、海外巨头全面转向训推分离，PD 分离已成行业共识" },
      "随着大模型从「训练」进入「推理」阶段，海外头部公司已经不再把训练和推理视为同一套硬件可以高效解决的问题，而是开始为推理场景专门设计下一代计算产品，并进一步将推理过程中的 Prefill（预填充）和 Decode（解码）两个阶段拆开，分别用不同硬件方案承载。",
      "NVIDIA 推出 Rubin CPX，采用 GDDR7，面向 Prefill 环节优化；同时 Feynman X3D 基于 Feynman GPU 整合 Groq LPU，采用 TSMC SoIC 工艺构建 3D 堆叠架构，面向下一代大模型推理。",
      "Google TPU v8 规划两个版本：TPUv8x「Zebrafish」专为 AI 推理设计，TPUv8ax「Sunfish」则针对 AI 训练优化。TPU v7 方面，Anthropic 向 Google 订购 100 万颗 TPU 用于部署云端算力，并持续向 Google TPU 的合作设计商 Broadcom 采购。",
      "OpenAI 与 Cerebras 达成 100 亿美元（750 兆瓦）长期算力订购协议，充分利用 Cerebras 的晶圆级架构构建异构计算集群，面向低延迟推理任务。华为昇腾 950 也包含两个版本：950PR 针对 Prefill 场景，内存带宽 1.6TB/s；950DT 面向 Decode 场景，内存带宽 4TB/s。",
      { image: "/notes/fangqing-global-inference-landscape.png", alt: "海外巨头与国产 AI 推理芯片架构布局", caption: "海外巨头与国产厂商正在围绕 Prefill / Decode 分离、晶圆级架构和高带宽存储重构推理芯片路线。" },
      { callout: "核心信号：推理不再是训练的附属品，而是需要独立架构设计的核心场景；Prefill 和 Decode 的分离部署，正在成为下一代 AI 基础设施的重要范式。" },
      { heading: "二、国内推理芯片创业窗口" },
      "海外巨头的技术路线转向，也给国内市场带来了新的创业窗口。近两年，国内涌现出一批专注于 AI 推理芯片的创业公司，包括遂源科技、清微智能（可重构计算流 RPU）、东方算芯（3D DRAM）、元川微（SRAM）、昉擎科技（SRAM + DRAM）等。",
      "这些公司切入角度各不相同，但共同判断是：GPU 并非推理场景的最优解，针对大模型推理的专用芯片存在显著效率提升空间。相较于海外公司已有的技术积累和市场先发优势，国内创业公司更多从特定技术路线或细分场景出发，在推理芯片赛道寻找差异化立足点。",
      { heading: "三、海外三条技术路线：Dojo、Groq、Cerebras" },
      "在 Transformer 大模型时代，英伟达长期一家独大。但近存架构代表 Dojo 和硅谷 AI 芯片独角兽的成功，给 AI 芯片产业指向了新的方向。",
      "Tesla Dojo 于 2023 年 7 月投产，到 2024 年底 Dojo 1 拥有相当于 8000 个 NVIDIA H100 GPU 的 AI 算力。Dojo 的核心特点是采用 SoW（System on Wafer）技术，将 25 颗芯片构成一个 Tile，并放在 12 英寸 Wafer 晶圆上互联。制程方面采用台积电 7nm 工艺，目前主要客户是特斯拉内部应用，未来可能对外供应。",
      "Groq 的路线是 LPU（Language Processing Unit）单芯片极致优化。2024 年 Meta 向 Groq 下了 1 亿美元订单用于部署 Meta Llama 3.1 大语言模型。Groq 片上存储容量 64GB，采用 SRAM 介质，制程为 14nm，强调小型高性能计算单元，通过软件优化提升推理效率。后续 4nm LPU 的性能演进值得继续关注，但 SRAM 在 4nm 以下的密度提升有限，是这条路线的结构性约束。",
      "Cerebras 的路线最激进：通过超大规模晶圆级设计，在整块晶圆上刻蚀电路。优点是计算规模极大，缺点是良率、散热和工程实现难度都非常高，片上存储介质同样是 SRAM。",
      "三家公司代表三个方向：Groq 强调小型高性能计算单元，通过软件优化提升推理效率；Cerebras 通过整块晶圆级设计提升计算规模；昉擎科技则希望结合两者优势，同样采用 SoW 方案，但通过小型核心单元组合形成大规模系统，在制造可实现性和系统扩展能力之间寻找平衡。",
      { heading: "四、SoW 晶圆级芯片：为什么互联是关键" },
      "要理解这些新架构为什么重要，需要回到一个底层逻辑：单颗芯片内部可以通过近存架构降低延迟，但要提升整体算力系统效率，芯片之间的互联才是更关键的瓶颈。",
      "传统方案中，英伟达 GPU 之间使用 NVLink 互联，介质是铜线。而 Tesla Dojo 和昉擎科技的做法是将 25 颗芯片构成一个 Tile 放在 12 英寸晶圆上，以晶圆本身作为互联基板，这就是 SoW（System on Wafer）技术。",
      "原来的芯片制造范式是：设计最小芯片，组装成模组，模组做成机匣，机匣组装成机架，机架组成系统。这种层层堆砌的实现方法，会把芯片通过先进工艺提升的性能，在工程实现中逐层衰减；系统规模越大，性能衰减越明显。",
      "SoW 方案直接跳过芯片切割、封装、互联、电缆、光缆等步骤，利用晶圆级的物理实现，充分发挥大规模、高密度和低损耗特点。一个直观类比是：CoWoS 是「切一小块晶圆当桥」，SoW 是「整张晶圆都是桥」。",
      ...
      "从底层架构看，其设计包含类似 LPU 的近存计算单元，通过大规模 SRAM 放置于计算核心附近，降低数据搬运延迟。同时公司采用重核架构，将计算、通信、互联和存储能力集成在核心系统中，再通过多个核心组合形成更大规模系统。昉擎科技更接近「系统级 AI Infra 公司」，而不仅是单颗 AI 芯片公司。",
      { heading: "七、存储方案、SoW 路线与商业化路径" },
      "昉擎科技创业初期认真评估过 3D IC 方案，认为目前还不够成熟：3D IC 现阶段只能堆容量到 48GB-64GB，继续往上堆良率会快速下降，且存在严重散热问题。",
      "因此公司目前采用片上 SRAM + 片外 GDDR/HBM 方案：用 SRAM 提供高带宽、低延迟的数据访问能力，同时外挂 DRAM 扩展容量，在成本和性能之间取得平衡。待 3D IC 技术成熟后，昉擎科技计划转向 SRAM + 片上 DRAM 方案，用 SRAM 存储权重，用 DRAM 存储 KV Cache。",
      "如果说 Groq 是「单芯片极致优化」路线，Cerebras 是「整颗晶圆级设计」路线，昉擎科技希望在两者之间找到平衡：同样采用 SoW（25 颗芯片构成一个 Tile，放在一整颗 12 英寸 Wafer 上），但通过小型核心单元组合形成大规模系统，兼顾制造可实现性和系统扩展能力。",
      "商业模式上，昉擎科技并不打算单纯销售芯片或板卡，而是提供服务器和集群级产品，把芯片、软件适配、系统调优整体交付给客户。第一阶段，公司计划通过异构方式降低客户采用门槛：客户已有 GPU 资源继续承担 Attention 计算，昉擎科技服务器负责 FFN 计算。",
      "这种渐进式方案的价值在于，大型互联网公司已有大量 GPU 存量资源，并不需要一次性替换整个体系。昉擎科技的异构方案是在现有基础设施上增加新的推理加速能力，理论上更容易被大型客户接受。",
      "公司第一代芯片已进入流片阶段，预计回片后进行测试验证。第一代产品主要面向云端推理场景，采用 12nm 工艺；后续计划推出面向端侧市场的新一代芯片，并规划台积电 6nm 制程，以满足更低功耗和更小尺寸需求。",
      { heading: "八、风险与判断" },
      "与后摩智能、元川微等传统 AI 芯片创业公司相比，后者更多聚焦于存储计算、SRAM 或特定计算架构优化，技术路线确定性相对更高，但也更容易进入已有竞争格局。昉擎科技的路线风险更高，但如果验证成功，上限可能更大。",
      "主要风险有三个：第一，技术验证，新架构必须通过真实客户的大模型推理任务验证，而不能停留在理论指标；第二，软件生态，AI 芯片的最终竞争不仅是硬件性能，还包括编译器、框架适配和开发环境；第三，规模交付能力，芯片公司最终需要从实验室产品走向大规模部署，对供应链和工程能力要求很高。",
      "总结来看，AI 推理正在经历一场架构范式变革。从海外巨头的 PD 分离布局，到 Dojo、Groq、Cerebras 三条技术路线崛起，行业共识正在形成：GPU 不是推理的终局，专用架构才是方向。昉擎科技提出的 Attention / FFN 分离、异构计算以及面向集群的系统级设计，本质上是在探索区别于传统 GPU 架构的新型推理基础设施。",
    ],
  },
  {
    id: 6,
  ...
  },
  {
    ...
    content: [
      ...
      "CPU（中央处理器）的设计是一个从顶层架构定义到物理实现的系统工程。整个流程从指令集架构（ISA）的选择与定义出发，经历微架构设计、RTL 编码、逻辑综合、物理版图设计，最终完成流片与封测。",
      "ISA 是软件与硬件之间的核心抽象层，规定处理器能够识别和执行的指令类型、寄存器组织、寻址模式及数据格式。理解 CPU，本质上要先理解 ISA 如何约束上层软件与底层硬件之间的接口。",
      { image: "/notes/riscv-cpu-design-flow.png", alt: "CPU 设计流程示意图", caption: "CPU 设计流程：从指令集定义、微架构设计，到 RTL、逻辑综合与物理设计。" },
      { image: "/notes/riscv-project-timeline.png", alt: "CPU 项目开发时间线", caption: "典型 CPU 项目从架构冻结、设计冻结、流片、回片到量产验证的工程节奏。" },
      { heading: "二、CISC 与 RISC：两种指令集哲学" },
      "主流指令集架构可分为 CISC（复杂指令集）与 RISC（精简指令集）两大阵营。CISC 以 x86 为代表，追求以少量复杂指令完成多步操作，指令长度可变，硬件解码复杂度高，但历史生态积淀深厚。",
      "RISC 以 ARM 和 RISC-V 为代表，采用固定长度指令与简化执行流水线，更易于实现高频率与低功耗设计。两者在设计复杂度、能效比、编译器优化空间等方面各有取舍，适用于不同应用场景。",
      { image: "/notes/riscv-isa-classification.png", alt: "CPU 指令集分类", caption: "CPU 指令集分类：x86 属于 CISC；ARM、RISC-V、PowerPC 等属于 RISC。" },
      { image: "/notes/riscv-mainstream-isa-comparison.png", alt: "x86、ARM 与 RISC-V 主流指令集对比", caption: "主流 ISA 对比：授权模式、模块化、可扩展性、功耗、应用场景、生态成熟度与优劣势。" },
      { heading: "三、x86、ARM 与 RISC-V 的位置" },
      "x86 以 Intel 与 AMD 为核心，拥有最成熟的软件生态与最强的单核性能表现，是当前服务器 CPU 的主流架构。x86 采用封闭授权模式，不对外开放 ISA 授权，第三方厂商进入壁垒极高。",
      "ARM 以 ARM Holdings、苹果、高通等为代表，是当前智能手机、平板等移动终端的绝对主流架构，并凭借低功耗与高核心密度特性加速向服务器 CPU 领域渗透。ARM 采用商业授权模式，授权费用较高，也存在地缘政治约束。",
      "RISC-V 最大特点是完全开源、免授权费且支持模块化扩展，被业界视为面向 AI 时代的原生架构。当前主要应用于 IoT 及边缘侧等轻量场景，整体软件生态仍处于早期建设阶段。对中国企业而言，其核心战略价值在于实现指令集层面的自主可控。",
      { heading: "四、为什么 RISC-V 值得持续跟踪" },
      "政策层面，RISC-V 已被纳入自主可控信息技术体系的重要技术路线。技术层面，RISC-V International 推广 RVA23 架构标准后，向量扩展（V 扩展）与虚拟化扩展（H 扩展）开始补齐服务器级特性，RISC-V 正在从嵌入式场景向高性能计算场景突破。",
      "产业层面，英伟达宣布 CUDA 将支持 RISC-V 架构 CPU 处理器，是 RISC-V 在 AI 计算生态中获得的重要背书。Google、高通、三星等国际巨头也在不同程度拥抱 RISC-V，说明它不再只是学术或嵌入式社区里的开放指令集，而是在进入更高性能、更大生态位的竞争。",
      { heading: "五、国内 RISC-V CPU 厂商观察" },
      "国内 RISC-V 架构 CPU 头部厂商包括阿里达摩院、北京开芯院、进迭时空、奕能科技、芯芯算力、睿思芯科、灵睿智芯等。不同厂商在 IP 自研、SoC 落地、边缘 AI、服务器级 CPU、融资进展和生态合作上各有侧重。",
      { image: "/notes/riscv-domestic-vendors.png", alt: "国内 RISC-V 架构 CPU 头部厂商情况", caption: "国内 RISC-V 架构 CPU 头部厂商：团队背景、产品业务、商业进展、融资估值与评价。" },
      { heading: "六、当前核心挑战" },
      "第一，软件生态仍是最突出的短板。工具链层面，调试与 Trace 工具支持不足，编译器代码生成与优化能力与 x86 / ARM 成熟工具链仍有显著差距；操作系统层面，SoC 厂商驱动主线化进程较慢，Linux 发行版往往需要维护内核分支，增加适配与维护成本。",
      "第二，开源带来了模块化自由，也带来了碎片化风险。RISC-V 允许厂商自由添加专有扩展指令，不同芯片之间的软件兼容性可能无法保证。ARM 通过严格授权和合规测试维持生态一致性，RISC-V 的 Profile 标准、合规性测试与认证体系仍在早期。",
      "第三，高性能领域仍有明显差距。RISC-V 在缓存层次设计、乱序执行深度、分支预测精度、内存一致性模型等关键指标上仍落后于 ARM 与 x86 的成熟设计。服务器和数据中心场景尚未出现大规模量产与商用部署案例，产品稳定性与可靠性仍需要时间验证。",
      "第四，商业模式还没有完全跑通。开源 ISA 并不等于免费芯片，RISC-V 企业仍需要长期投入微架构、SoC、软件栈和生态适配。当前行业尚未出现类似 ARM Cortex-A 系列的标杆级 IP，也缺少足以定义应用场景的明星级 SoC。",
      { heading: "七、一级市场观察框架" },
      "RISC-V 的投资判断不能只看「开源」或「自主可控」叙事，核心仍要看谁能在具体场景里建立芯片 + 软件 + 应用的闭环。值得重点关注三类标的：一是掌握高性能 CPU 核 IP 自研能力的头部厂商；二是围绕 RISC-V 软件生态建设的工具链与操作系统企业；三是率先在垂直场景（汽车电子、AI 推理、数字基础设施）实现量产落地的应用层公司。",
      "个人判断，RISC-V 仍处于从 IoT 嵌入式向高性能计算突破的爬坡阶段。它的战略价值明确，但真正跑出来的公司，不会只靠 ISA 开源，而要靠高性能微架构、稳定软件生态、客户场景绑定和持续工程交付能力共同成立。",
    ],
  },
  {
    id: 2,
